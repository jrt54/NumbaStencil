...
def timestep_stencil(curr_timestep, prev_timestep, coeff):
	const = (dt*vel/h)**2
	new_timestep = const*coeff[0]*curr_timestep[0, 0, 0]*3
	for i in prange(1, radius+1):
		new_timestep+= const*coeff[i]*((curr_timestep[0, 0, -i] + curr_timestep[0, 0, i]) + (curr_timestep[0, -i, 0] + curr_timestep[0, i, 0]) + (curr_timestep[-i, 0, 0] + curr_timestep[i, 0, 0]))
	##end laplacian
	#next_timestep*=((dt*vel/h)**2)
	new_timestep += 2*curr_timestep[0,0,0] - prev_timestep[0, 0, 0]
...
def time_step(new_timestep, curr_timestep, prev_timestep, nt):
	for i in range(1, nt): 
		#next_timestep = 2*curr_timestep - prev_timestep + laplace(curr_timestep, coeff)*((dt*vel/h)**2) 
		
		new_timestep = timestep_stencil(curr_timestep, prev_timestep, coeff)
		new_timestep[x_idx+radius, y_idx+radius, z_idx+radius] += source[i]*(vel*dt)**2
		prev_timestep = curr_timestep
		curr_timestep = new_timestep
		print(new_timestep[x_idx+radius, y_idx+radius, z_idx+radius])
	return new_timestep
